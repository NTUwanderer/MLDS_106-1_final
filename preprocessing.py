...
def parse_annotation(ann_dir, img_dir, input_size_h, input_size_w, labels=[]):
    all_imgs = []
    ...
    for pair in read_data.getRealPairs():
        ...
        label = 0
        json_data = json.load(open(pair[1]))

        if 'L' in json_data['bbox']:
            coord = json_data['bbox']['L']
            obj = {'name': 'L', 'xmin': coord[0], 'ymin': coord[1], 'xmax': coord[2], 'ymax': coord[3]}
            img['object'].append(obj)
            label += 1
        if 'R' in json_data['bbox']:
            coord = json_data['bbox']['R']
            obj = {'name': 'R', 'xmin': coord[0], 'ymin': coord[1], 'xmax': coord[2], 'ymax': coord[3]}
            img['object'].append(obj)
            label += 2

        seen_labels[labels[label-1]] += 1
